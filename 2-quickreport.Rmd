...
Our [2010 Nature paper](http://www.nature.com/nature/journal/v464/n7288/full/nature08930.html) concluded by saying yeah, R~S~ is increasing globally, based on an analysis of a soil respiration database (SRDB), **but** we can't distinguish whether this is an acceleration of the C cycle, or a feedback:
...
Seven years later, we have a _lot_ more data, both within the SRDB and from other sources. How can we test whether R~H~ is in fact increasing? Various lines of evidence:
...
So yes, there seem to be continuous increases with time in the relationship between R~H~ and R~S~--i.e., more recent studies exhibit higher ratios. This is what we'd expect to see if R~H~ were increasing due to a climate feedback (SOC loss).

### 2. R~H~ and R~S~ versus GPP

GPP is the ultimate source for both the autotrophic and heterotrophic soil surface CO~2~ fluxes, and measured [RS is related to GPP](http://www.biogeosciences.net/7/1915/2010/). If SOC is being increasingly lost, i.e. R~H~ is increasing, we'd expect to see both the R~H~:GPP and R~S~:GPP ratios increase over time.

There are three (?) sources of GPP data. **The first source** is the [Beer et al. 2010](http://www.sciencemag.org/cgi/content/short/329/5993/834) dataset.
...
Both the RS/gpp and RH/gpp ratios show increases over time, even after taking into account climate normal effects.

The **second GPP data source** is within the SRDB itself, i.e. as reported by study authors.
...
The **third source of GPP data** is the MODIS GPP product. This seems like a PITA so I haven't worked with this yet.

### 3. Climate effects on R~H~

Following the logic of the 2010 Nature paper, we'd also expect to see climate anomalies associated with higher RH values, relative to the climate-normal baseline.
...
Interestingly, while R~S~ continues to show changes driven by temperature anomalies (not shown), R~H~ is affected by the precip norm (i.e. 1961-1990); the precip anomaly (responding positively); and year (there's a positive trend for unknown reasons). We don't see any temperature effect. This dataset is significantly larger than the 2010 one (N=`r nrow(subset(d, !is.na(Rh_annual)))` now versus 163 then).

### Summary

Test  | N | Result consistent with R~H~ feedback?
------------- |  ------------- | -------------
Changing R~H~:R~S~ ratio over time  | 207 | Yes
Changing R~H~:Beer GPP over time  | 215 | Yes
Changing R~S~:Beer GPP over time  | 1283 | Yes
Changing R~H~:study GPP over time  | 20 | No: no trend
Changing R~S~:study GPP over time  | 101 | No: no trend
Changing R~H~:MODIS GPP over time  | - | -
Changing R~S~:MODIS GPP over time  | - | -
R~H~ trend over time | 216 | Yes

### Caveats and next steps

Caveats:

* Stitching together a bunch of things here--no single one of them is definitive proof.
* Collectively though, they're suggestive.
* These regression tests are not very sophisticated (yet).
...
* What other tests can we bring to bear/apply?
